...
namespace TaskbarIconHost
{
    ...
    {
        /// <summary>
        /// Plugin name. This value is read once before <see cref="Initialize"/> is called, and cached. Null or an empty string is not allowed, in this case the plugin will not load.
        /// </summary>
        string Name { get; }

        /// <summary>
        /// Plugin unique ID. This value is read once before <see cref="Initialize"/> is called, and cached. The empty guid is not allowed, in this case the plugin will not load.
        /// </summary>
        Guid Guid { get; }

        /// <summary>
        /// Indicate if the plugin require elevated (administrator) mode to operate. This value is read once before <see cref="Initialize"/> is called, and cached.
        /// </summary>
        bool RequireElevated { get; }

        /// <summary>
        /// Indicate if the plugin want to handle clicks on the taskbar icon. This value is read once before <see cref="Initialize"/> is called, and cached. This setting only has a minor impact on the UI, since the <see cref="OnIconClicked"/> event handler is called nevertheless.
        /// </summary>
        ...
        /// <summary>
        /// Request to begin closing the plugin. This request is sent if all plugins have returned true to the <see cref="CanClose"/> request.
        ...
        /// <summary>
        /// Check if the plugin is closed. See <see cref="CanClose"/> and <see cref="BeginClose"/>.
        /// </summary>
        bool IsClosed { get; }
    }
}
